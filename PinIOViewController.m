    ...
    for (int i = 0; i < length; i+=3){
        ...
        //Analog Reporting (per pin)
           ...
           // else if (data[i] == 0xe0){
            ...
            int val = data[i+1] + (data[i+2]<<16);
           // int val = data[i];
 

            
     
           CGFloat Sat= ( val & 0xff) /255.0; //1024.0f
            
            CGFloat Bright= 0.8;
            
         UIColor *backGroundColor=[UIColor colorWithHue:0.12 saturation:Sat brightness:Bright alpha:1.0];
...
//    }
     ...
     if (indexPath.section == ANALOG_PIN_SECTION){  //Analog Pins A0-A5
...
//    }
    if (indexPath.section == ANALOG_PIN_SECTION){
